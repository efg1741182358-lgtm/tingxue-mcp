...
// 没配就**不注入**，而不是替使用者挑一个地址。
//
// 这里曾经写死一个示例 IP 兜底。那意味着每一份忘了配的部署都从同一个
// 地址出站，那个地址迟早被限流，然后是所有人一起挂、各自都查不出原因。
// 更糟的是它在「替别人做决定」：伪造出口 IP 这件事，使用者有权知道自己
// 在做，不该由一个默认值悄悄替他做了。
//
// 没配的后果是可见的：境外部署会拿到 -460，而 explain() 会明确说
// 「这几乎总是 IP 限区，去设 REAL_IP」。报错好过静默地用错地址。
const REAL_IP = (process.env.REAL_IP || '').trim()

if (!REAL_IP) {
  console.warn(
    '[warn] 未设置 REAL_IP，本次不注入出口 IP。\n' +
    '       服务器在境外时网易云会按 IP 限区，返回空结果或 -460。\n' +
    '       境外部署请填一个国内 IP；本机就在国内则无需设置。'
  )
}
